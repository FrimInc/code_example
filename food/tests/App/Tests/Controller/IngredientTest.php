...
class IngredientTest extends WebTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        static::$obType          = self::$obTypeRepository->findBy([], ['sort' => 'asc'])[0];
        static::$obType2         = self::$obTypeRepository->findBy([], ['sort' => 'asc'])[1];
    ...
    public function testIngredientDelete(): void
    {
        ...
        $this->assertSame(ExceptionService::NO_ACCESS_EDIT['code'], $arResponseData['code']);
    }
    ...
    public function testIngredientPutExists(): void
    {
        ...
        $this->assertArrayHasKey('status', $arResponseData);
        $this->assertSame(false, $arResponseData['status']);
        $this->assertSame(ExceptionService::INGREDIENT_NAME_EXISTS['code'], $arResponseData['code']);
    }
    ...
    public function testIngredientTypeTwoo(): void
    {
        ...
        for ($intPage = 0; $intPage <= 120; $intPage++) {
            $this->client->request(
                'POST',
                '/app/ingredients',
                [
        ...
        $this->assertSame(2, count($arFoundType), print_r([$arFoundType, $arCheckTypes, $_SERVER], true));
    ...
    {
        $obExistsIngredient = self::$obIngredientRepository->findByName('Сахар', self::$obRegularUser)[0];
        ...
        $arResponseData = json_decode($obResponse->getContent(), true);
        $this->assertArrayHasKey('status', $arResponseData);
        $this->assertSame(false, $arResponseData['status'], print_r([$obExistsIngredient->getId(), $arResponseData], true));
    }
    ...
    public function testIngredientAutocomplete(): void
    {
        ...
        $this->assertJson($obResponse->getContent());
        $arResponseData = json_decode($obResponse->getContent(), true);
        $this->assertStringContainsStringIgnoringCase('Саха', $arResponseData[0]['name']);
    }
